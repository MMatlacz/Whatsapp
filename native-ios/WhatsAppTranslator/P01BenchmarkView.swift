...
struct P01BenchmarkView: View {
    ...
    var body: some View {
        List {
            Section("Model gate") {
                diagnosticRow("Availability", value: runner.modelAvailability)
                diagnosticRow("Context size", value: "\(runner.model.contextSize) tokens")
                diagnosticRow("Supported languages", value: "\(runner.model.supportedLanguages.count)")
                diagnosticRow("Indonesian (id_ID)", value: runner.model.supportsLocale(Locale(identifier: "id_ID")) ? "advertised" : "not advertised")
                diagnosticRow("Polish (pl_PL)", value: runner.model.supportsLocale(Locale(identifier: "pl_PL")) ? "advertised" : "not advertised")
            }
